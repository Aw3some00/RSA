...
#include <cstdlib>
#include <ctime>
std::vector<int> consequence(2000);
std::vector<int> Eyler_result(2000);
...
std::vector<int> normalizeVector(std::vector<int> vec) {
...
std::vector<int> normalizeVectorlink(std::vector<int>&vec) {
    while (!vec.empty() && vec.back() == 0) {
        vec.pop_back();
    }
    if (vec.empty()) {
...
std::vector<int>& eyler_function(std::vector<int> key1,std::vector<int> key2) {
    ...
    key1=key1-s1;
    key2=key2-s1;
    Eyler_result=key1*key2;
    normalizeVectorlink(Eyler_result);
    return Eyler_result;
}
std::vector<int>& secret_exponent(std::vector<int> &key1,std::vector<int> &key2,int e,std::vector<int> &s1,std::vector<int>&s2){
    std::vector<int> single={1};
    std::vector<int> e_in_vector=IntTovector(e);
    
    eyler_function(key1,key2);
    std::vector<int> random_number={2};
    consequence=(((random_number*Eyler_result)+single)/e_in_vector);
    normalizeVectorlink(consequence);
    return consequence;
}
...
int main() {
    std::string num1 = "17";//389 503
    std::string num2 = "23";//467 509
    ...
    std::vector<int> multiply=vector1*vector2;
    std::cout<<"Подождите,идет шифрование \n";
    std::vector<int> scrypt=(message_scrypt^e1)%(multiply);
    std::cout<<"\n";
    std::cout<<"Зашифрованный текст \n";
    printVector(scrypt);
    std::cout<<"Подождите, идет расшифровывание \n";
    std::vector<int>& secret=secret_exponent(vector1, vector2,e1,vector1,vector2);
    std::vector<int> descypt=(scrypt^secret)%(multiply);
